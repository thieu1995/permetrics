...
def test_regression_metrics(rm_data, metric_name):
    ...
    if dims == 1:
        ...
        assert isinstance(
            res, float
        ), f"{metric_name} failed on 1D data. Expected float, got {type(res)}"
...
def test_sample_wise_regression_metrics(rm_data, metric_name):
    ...
    if isinstance(rm.y_true, (int, float)):
        assert isinstance(
            res, (float, int, list, tuple, np.ndarray)
