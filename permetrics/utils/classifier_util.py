...
def calculate_single_label_metric(matrix, imap, imap_count, beta=1.0):
    ...
    with np.errstate(all="ignore"):
        for label, idx in imap.items():
            ...
            metric["negative_predict_value"] = np.nan_to_num((tp + tn) / (tp + tn + fp + fn), nan=0.0, posinf=0.0, neginf=0.0)
            metric["accuracy"] = np.nan_to_num(tn / (tn + fn), nan=0.0, posinf=0.0, neginf=0.0)
